...
class EventController extends Controller
{
    public function addEvent(EventRequest $request)
    {
        ...
        foreach ($parentsToNotify as $parent) {
            Notification::send($parent, new EventNotification($event, $SC, " تم اضافة مناسبة جديدة من قبل " . $school->name, "/Darb/ShowGrade/Event/" . $event->id));
        }
    ...
    public function updateEvent(EventRequest $request, $id)
    {
        ...
        $parentsToNotify = User::whereIn('id', $parents)->get();
        foreach ($parentsToNotify as $parent) {
            Notification::send($parent, new EventNotification($event, $SC, "تم تعديل مناسبة من قبل  " . $school->name, "/Darb/ShowGrade/Event/" . $event->id));
        }
